    ...
    const onFormSubmit = useCallback((event: any): void => {
        ...
        if (additions && updateAdditions && deletions && reducedApplicationStatus && reducedDeployment && application && submissionErrorModal.show) {
            ...
            try {
                ...
                submissionErrorModal.show("You cannot queue the installation of an application on a subtree with nodes that are queued for removal");
            }
        }
    }, [additions, updateAdditions, deletions, reducedApplicationStatus, reducedDeployment, application]);
 ...
 *
 * TODO: Throw error iff the status map record is a removal record
 *
