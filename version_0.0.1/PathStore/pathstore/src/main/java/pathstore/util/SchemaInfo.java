...
public class SchemaInfo {
	static private SchemaInfo instance=null;
	
	static public SchemaInfo getInstance() {
		if (SchemaInfo.instance == null) 
			SchemaInfo.instance = new SchemaInfo();
	...
	HashMap<String,HashMap<Table,List<Column>>> schemaInfo = new HashMap<String,HashMap<Table, List<Column>>>(); 
	...
	private void loadSchemas() {
		...
		try {
			...
			for (Row row : results) {
				...
				if (keyspace_name.startsWith("pathstore_"))
					getKeySpaceInfo(keyspace_name);
			}
	...
	public HashMap<Table,List<Column>> getKeySpaceInfo(String keyspaceName) {
		if (schemaInfo.get(keyspaceName) != null)
			return schemaInfo.get(keyspaceName);
		...
		try {
			...
			PreparedStatement prepared = session.prepare("select * from system_schema.tables where keyspace_name=?");
			...
			for (Row row : results) {
				...
				Column column = new Column(keyspace_name, table_name, column_name,
						clustering_order, kind, position, type);
	...
	private List<Column> getColumns(HashMap<Table, List<Column>> keyspaceInfo, String tableName) {
		for (Table t : keyspaceInfo.keySet()) {
			if (t.table_name.compareTo(tableName)==0)
				return keyspaceInfo.get(t);
		}
		return null;
	}
	...
	public static class Column {
		...
		public Column(String keyspace_name, String table_name, String column_name,
				String clustering_order, String kind, int position, String type) {
			super();
