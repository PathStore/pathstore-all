...
public class PathStoreServerClient {
	...
	public void addQueryEntry(QueryCacheEntry entry) {
		try {
			...
			String result = stub.addQueryEntry(entry.getKeyspace(), entry.getTable(), entry.getClausesSerialized(), entry.limit);
	...
	public void addCommandEntry(String device, QueryCacheEntry entry) throws PathMigrateAlreadyGoneException,  PathStoreRemoteException{
		//try {
			String result;
			try {
				result = stub.addUserCommandEntry(device, entry.getKeyspace(), entry.getTable(), entry.getClausesSerialized(), entry.limit);
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
	...
	public UUID cretateQueryDelta(QueryCacheEntry entry) {
		try {
			return stub.createQueryDelta(entry.getKeyspace(),entry.getTable(), 
										 ...
										 PathStoreProperties.getInstance().NodeID, entry.limit);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			throw new PathStoreRemoteException();
