...
//		Session session2 = cluster2.connect();
		...
		insert.value("name", name);
		insert.value("sport", "test2");
		insert.value("years", 445);
		insert.value("vegetarian", true);
...
//		Thread.sleep(1000);
		...
		for(Row row : results)
		{
			System.out.println(row.getUUID("pathstore_parent_timestamp"));
		}
