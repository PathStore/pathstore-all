...
//
///** TODO: Currently Not used Most likely WIP */
...
//    String only =
//        (String)
//            config.getProperty("onlyDifferences"); // "/home/mortazavi/nfs/traces/trace-asli.txt";
//    keyspace = (String) config.getProperty("keyspace");
//    System.out.println("(((((((((((((((((( " + only);
//    if (only.equals("false")) onlyDifferences = false;
//
...
//    if (PathStoreProperties.getInstance().role == Role.SERVER) getConnectionWithParent();
//  }
//
...
//
//      sessionLocks.compute(sid, (k, v) -> serverHanldeCookie(k, v, previousEdge));
//
...
//    //		System.out.println("((((((((sid: " + sid + " status: " +status + " prevEdge: "+
//    // previousEdge);
//
//    if (status == null) {
...
//              if (ce == null) ce = createNewNeighborConnection(previousEdge);
//              int parentId = PathStoreProperties.getInstance().ParentID;
//
//              try {
//                System.out.println(
//                    "connecting to src PathstoreMigrate at: " + ce.RmiIP + ":" + ce.PortRmi);
...
//
//                  System.out.println(
//                      "sending out only differences differenceList size: "
//                          + differenceList.get("users").size());
...
//                  System.out.println(
//                      "######processing time for difference list at destination: "
//                          + Timer.getTime(dd));
//                }
...
//                  }
//
//                  ArrayList<CommandEntryReply> replies =
//                      ce.stub.sendData(
//                          sid,
...
//                          + Timer.getTime(howisthispossible)
//                          + " "
//                          + replies.size());
//                  System.out.println(
//                      "&&&&&&&! sending data from " + sid + " at: " + Timer.getTime(d));
//
//                  if (onlyDifferences || sid.equals("moveall")) {
//                    if (sid.equals("moveall")) {
//                      long currentTotalBytes = LoggingHelperClass.getTotalBytes("neighbor");
//                      System.out.println(
//                          "^^^^^^^^ FINAL total bytes for transfer: "
...
//                      System.out.println(
//                          "MMMMMMMMM: " + timme + " " + wastedDst + " " + wastedOnSrc);
//                      System.out.println("^^^^^^^^ FINAL totalTime = " + finalTime);
//                      System.out.println(
//                          "^^^^^^^^ FINAL total bytes for transfer: "
//                              + (currentTotalBytes - prevTotalBytes));
//                    }
//                  }
//
...
//                    tmp = System.nanoTime();
//                    QueryCache.getInstance()
//                        .updateCacheByMigration(
//                            rr.getKeyspace(), rr.getTable(), rr.getConverted(), rr.getLimit());
//                    queryTime += (System.nanoTime() - tmp) / 1000;
//                    // System.out.println("query " + i++ +" took: " + (System.nanoTime()-tmp)/1000);
//                  }
//                  System.out.println(
//                      "&&&&&&&&&! done updating caches at: "
//                          + Timer.getTime(d)
//                          + "  query: "
//                          + (queryTime / 1000.0)
//                          + " command: "
...
//                  ArrayList<CommandEntryReply> replies =
//                      ce.stub.sendData(
//                          sid, PathStoreProperties.getInstance().NodeID + "", true, differenceList);
//                  System.out.println(
//                      "got this many replies: (QCentries): "
//                          + replies.size()
//                          + " /"
//                          + Timer.getTime(d));
//                  long tmp = System.nanoTime();
//                  ArrayList<CommandEntryReply> differences =
//                      QueryCache.getInstance().reconsolidateWithNeighbor(replies);
//                  System.out.println(
//                      "after reconsilidation "
//                          + Timer.getTime(d)
//                          + " need to change: "
//                          + differences.size());
//                  if (differences != null && differences.size() > 0)
//                    parentServerStub.fetchFromParent(
//                        differences, PathStoreProperties.getInstance().NodeID + "");
//                  queryTime += (System.nanoTime() - tmp) / 1000;
//                  System.out.println(
//                      "&&&&&&&&&! done updating caches at: "
//                          + Timer.getTime(d)
...
//                    "&&&&&&&&&&&&&&! setting "
//                        + sid
//                        + " current edge session: "
//                        + Timer.getTime(d));
//                setSessionCurrentEdge(sid);
//                System.out.println(
//                    "&&&&&&&&&&&&&&&&&! finished moving " + sid + " took: " + Timer.getTime(d));
...
//
//      return "MOVING";
//    } else if (status.equals("MOVING")) return "MOVING";
//    else if (status.equals("MOVED")) return "MOVED";
//
//    return null;
...
//    if (r == null) {
//      Insert ins =
//          QueryBuilder.insertInto("pathstore_applications", "session")
//              .value("sid", sid)
...
//  }
//
//  public ArrayList<CommandEntryReply> sendData(
//      String sid, String dstEdge, boolean neighbors, Map<String, Map<Object, UUID>> rowsOnDest) {
//
...
//      System.out.println(
//          "^^^^^^^ total bytes for delta list transfer: " + (currentTotalBytes - prevTotalBytes));
//    }
//
...
//
//    // TODO Auto-generated method stub
//    HashMap<String, HashMap<String, HashMap<String, List<QueryCacheEntry>>>> commands =
//        QueryCache.getInstance().getDeviceCommands();
...
//    if (ce == null) ce = createNewNeighborConnection(dstEdge);
//
//    Session dstSession = pathStoreConnectionCache.get(dstEdge);
//    if (dstSession == null) {
//      Cluster dst =
//          Cluster.builder().addContactPoints(ce.PathStoreIP).withPort(ce.PathStorePort).build();
...
//      System.out.println(
//          "TOTAL TIME WASTED BEFORE ACTUALLY STARTING ********* DEDUCT IF ONLY DIFFERENCES: "
//              + wasted);
//    }
...
//      ConcurrentMap<Table, Collection<Column>> tables =
//          SchemaInfo.getInstance().getColumnInfo().get(keyspace);
//      int allRowsNum = 0;
//      for (String table : tableNames) {
//        if (table.startsWith("view_")) continue;
//        Table t = null;
...
//        Collection<Column> columns = tables.get(t);
...
//            }
//
//            if (keyspace.equals("pathstore_applications")
//                && table.equals("session")
//                && c.column_name.equals("current_edge")) {
...
//                replies.add(
//                    new CommandEntryReply(
//                        keyspace,
//                        sid,
//                        table,
//                        cache_entry.getClausesSerialized(),
//                        cache_entry.limit));
//
//                Select selectt = QueryBuilder.select().all().from(keyspace, table);
//                selectt.allowFiltering();
//
//                for (Clause clause : cache_entry.getClauses()) selectt.where(clause);
//
//                if (cache_entry.limit > 0) selectt.limit(cache_entry.limit);
//
//                // System.out.println(" select is: " + selectt.toString());
//                long d = System.nanoTime();
//                long checkAndAddTime = 0;
//                // PathStoreResultSet results =  (PathStoreResultSet)
//                // localSession.executeLocal(selectt,null);
//                // hosseinhere:
//                selectt.setFetchSize(1000);
//                PathStoreResultSet results =
//                    new PathStoreResultSet(localSession2.execute(selectt), keyspace, table, false);
...
//                    // System.out.println(row.getObject(primary) + "  " +  row_timestamp + " " +
//                    // timestamp_on_dest);
//
//                    if (timestamp_on_dest != null
//                        && timestamp_on_dest.timestamp() >= row_timestamp.timestamp()) {
...
//                // System.out.println("creating +   " + ccnt +" insert entries and execution took: "
//                // +Timer.getTime(d) + " total time in checkAndAdd: " + checkAndAddTime + "   total
//                // time in insert: " + insertTime);
...
//          System.out.println(
//              "total inserted = " + totalInserted + " total skipped: " + totalSkippedRows);
//
...
//            + (Timer.getTime(totalTime) - defaultTime));
...
//      e.printStackTrace();
//    }
//    if (stub == null) System.err.println("new stub to neighbor is null");
//
//    ConnectionEntry ce =
//        new ConnectionEntry(destParentId, stub, null, dstIpRMI, dstPortRMI, dstIpRMI, dstPortPS);
//
...
//  }
//
//  public static BatchStatement checkAndAdd(
//      Statement statement, BatchStatement batch, Session dstSession, AtomicInteger batchSize) {
//
//    //		dstSession.execute(statement);
//    //		return batch;
//    int length = statement.toString().length();
//    if (length > 81920) dstSession.execute(statement);
//    else {
...
//  }
//
//  public static void checkAndAddUpdated(
//      Statement statement, Session sessionWriter, List<ResultSetFuture> tasks) {
//    ResultSetFuture future = sessionWriter.executeAsync(statement);
//    tasks.add(future);
//    if (tasks.size() < 40000) return;
//    for (ResultSetFuture t : tasks)
//      try {
...
//      if (tableName.startsWith("view_")) continue;
//      String primary =
//          cluster
//              .getMetadata()
//              .getKeyspace(keyspace)
...
//
//      // DataType type =
//      // cluster.getMetadata().getKeyspace("pathstore_demo").getTable("users").getPrimaryKey().get(0).getType();
//
...
//        // UUID row_timestamp = row.getUUID("pathstore_version");
//
//        //				if (highest_timestamp == null || highest_timestamp.timestamp() <
...
//  public String fetchFromParent(ArrayList<CommandEntryReply> queriesToExecute, String previousEdge)
//      throws RemoteException {
//    // TODO Auto-generated method stub
//
//    if (queriesToExecute == null) return null;
//
//    if (queriesToExecute.size() == 0) return null;
//
...
//
//      ConcurrentMap<Table, Collection<Column>> tables =
//          SchemaInfo.getInstance().getColumnInfo().get(queriesToExecute.get(0).getKeyspace());
//
//      ConnectionEntry ce = connectionCache.get(previousEdge);
//      if (ce == null) ce = createNewNeighborConnection(previousEdge);
//
...
//        for (Clause clause : clauses) selectt.where(clause);
//        if (rr.getLimit() > 0) selectt.limit(rr.getLimit());
//        else System.out.println("limit was: " + rr.getLimit());
//        selectt.allowFiltering();
...
//
//        PathStoreResultSet results =
//            new PathStoreResultSet(bb, rr.getKeyspace(), rr.getTable(), false);
...
//            }
//
//            if (rr.getKeyspace().equals("pathstore_applications")
//                && rr.getTable().equals("session")) // && c.column_name.equals("current_edge"))
//            {
//              // needs to be checked
//              continue;
//            }
...
//              insert.value(c.column_name, row.getObject(c.column_name));
//          }
//          // System.out.println("inserting for row: " + co++ + " before check and update took : " +
//          // Timer.getTime(dd));
//          checkAndAddUpdated(insert, dstSession, tasks);
